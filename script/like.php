...
if (isset($_SESSION['login']) && $_SESSION['login'] != "")
{
    ...
    try {
        ...
        $sql = $db->prepare("SELECT id_post, login FROM likes WHERE id_post = ? AND login = ?");
        $sql->execute(array($postid, $login));
        $has_liked =  $sql->rowCount();

        if ($has_liked === 0)
        {
            $update = $db->prepare("INSERT INTO likes(id_post, login) VALUES(?,?)");
        ...
        else if ($has_liked >= 1)
        {
            ...
            $upd = $db->prepare("DELETE FROM likes WHERE id_post = ? AND login = ?");
